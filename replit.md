...
- **Tiny ERP Integration:** Complete integration for automatic product data retrieval via SKU, with visual feedback, concurrency protection, auto-filling of product data (GTIN, dimensions, weight, pricing). Supports three operation modes: "Tiny Mode" (full auto-pricing with cost price from Tiny), "Hybrid Mode" (Tiny data without cost price, manual pricing calculation), and "Manual Mode" (no Tiny integration).
- **Text-to-Speech (TTS):** Features real-time word highlighting, auto-scrolling, and fixed playback controls.
- **Content Generation & Regeneration:** Offers reset functionality, proper combination of current and new prompt information for regeneration, and version history for FAQ, Cards, and Prices (aggressive and promo).
- **Visual Feedback:** Standardized sparkle icons for regeneration, document icons for copying (with state changes), checkmark feedback for all copy actions, and version control navigation (prev/next buttons with counters) for titles, descriptions, FAQ, Cards, and Prices.
- **Pricing UI:** Displays "Preço do Anúncio" (promo price / 0.85) with blue highlighting and three detailed analysis metrics (% Margem, Múltiplo de Valor, Valor Monetário) with 2 decimal places per price field (announcement, aggressive, promo). Prices are editable, with manual edits triggering metric recalculation. Visual warnings (red highlighting) for negative margins or low value multiples. In "Manual Pricing Mode," calculations are triggered by a "Calcular Preços" button, with specific alerts for Mercado Livre shipping costs.
...
- **State Management:** `integrationMode` dynamically switches between "manual", "tiny", and hybrid mode based on token availability, SKU input, and cost price validation. When Tiny ERP returns product data without a cost price, the system enters hybrid mode: product fields (GTIN, dimensions, weight) are auto-filled and read-only, but pricing fields remain editable with manual calculation via button.
- **Price Versioning:** Full version control system for aggressive and promotional prices (both Min and Max tabs). Each price change (manual or auto-calculated) creates a new version storing the price value and all associated metrics (margin %, value multiple, monetary value, taxes, commissions). Users can navigate between versions using prev/next buttons without backend calls. Versions are persisted in localStorage and restored on page load.
