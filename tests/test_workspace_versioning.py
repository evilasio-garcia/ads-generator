...
def test_merge_append_only_title_keeps_both_user_versions():
    ...
    simple = merged["variants"]["simple"]
    assert simple["title"]["versions"] == ["T1", "T2-user1", "T2-user2"]
    assert simple["title"]["current_index"] == 2
...
def test_merge_faq_lines_preserves_history_and_latest_pointer():
    ...
    merged = _merge_versioned_state(current, incoming)
    simple = merged["variants"]["simple"]
    assert simple["faq_lines"][0]["versions"] == [
        {"q": "Q1", "a": "A1"},
        {"q": "Q2-u1", "a": "A2-u1"},
        {"q": "Q2-u2", "a": "A2-u2"},
    ]
    assert simple["faq_lines"][0]["current_index"] == 2
    assert simple["faq_lines"][0]["approved"] is False


def test_normalize_state_legacy_payload_maps_to_simple_variant_and_drops_prices():
    raw = {
    ...
    simple = normalized["variants"]["simple"]
    assert simple["title"]["current_index"] == 1
    assert simple["description"]["current_index"] == 0
